...
$app->get('/acdb/pricesche', function(Request $request, Response $response){
	
	$id = $request->getAttribute('id');
	$sql = "select MATERIAL_NAME, MATERIAL_DETAIL, THB  from SCHEDULE_PRICE";
...
$app->get('/acdb/priceschedule/{id}', function(Request $request, Response $response){
	
	$id = $request->getAttribute('id');
	$sql = "select MATERIAL_NAME, MATERIAL_DETAIL, USD, THB  from SCHEDULE_PRICE WHERE KIND_NUM = '$id'";
...
$app->get('/acdb/matkind', function(Request $request, Response $response){
	
	$sql = "SELECT ID, KIND_NAME FROM  MAT_KIND_STD";
...
$app->get('/acdb/pricecmpkg', function(Request $request, Response $response){
	
	$sql = "select T1.compound AS CP, T1.RM as RM, ( T1.weigh / T2.SUM_WEIGH) as RM_KG from compound T1, (SELECT compound, sum(weigh) as SUM_WEIGH FROM compound group by compound) T2 where T1.compound = T2.compound";
...
$app->get('/acdb/distinct_material', function(Request $request, Response $response){
	
	$sql = "SELECT distinct material FROM ACDB.article_spec"; //From article spec
...
$app->get('/acdb/artspec/{art}', function(Request $request, Response $response){
	...
	$sql = "SELECT article, part, part_name, material, weigh FROM ACDB.article_spec where article = '$art'"; //From article spec
